...
export async function load({ cookies, fetch, url }) {
  ...
  if (role === "USER") {
    ...
    if (selectedDoctorId) {
      doctorAvailability = await UserService.getDoctorAvailability(
        token,
        selectedDoctorId,
      );
    }
  ...
  if (role === "DOCTOR") {
    ...
    if (visitIdFromUrl) {
      const currentVisit = dashboardData.visits.find(
        (v: any) => v._id === visitIdFromUrl,
      );

      if (currentVisit && currentVisit.patientId) {
        treatments = await DoctorService.getPatientTreatments(
          token,
          currentVisit.patientId,
        );
        patientVisits = await DoctorService.getPatientVisits(
          token,
          currentVisit.patientId,
        );
      }
